...
{
	NodePos _pos;
	_pos = ReadLocFile(satelliteId,GetTime());
	return _pos;
}
...
bool getInfStat(int satId,int interfaceIndex)
{
	int srcNodeId = satId;
	int dstNodeId = -1;
	Isl isl = G.isl[ Int_DB[satId-1].linkId[interfaceIndex] -1];
	for(int i = 0; i < 2; i++)
	{
...
void updateLink()
{
	...
	// 	{
	// 		Int_DB[i].port_stat[j] = getInfStat(i+1,j);

	// 	}
			
	// }
	// normalTopoChange = true;
	// UpdateOpspfRoutingTable();	
}

void updatePos()
{
	setPos(ReadLocFile(satelliteId,GetTime()));
}

void setPos(NodePos _pos)
{
	pos = _pos;
}
